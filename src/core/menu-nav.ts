...
// Relative import: src/core/ is shared between the test tsconfig
// (which has the `@/shared/*` path alias) and the emitting
// tsconfig.node.json (which doesn't). Sticking to relative keeps
...
import type { MenuConfig, MenuNode, TwistCyclePriority } from '../shared/menu.js';
import type {
  ShapeLayout,
  ShapePluginModule,
  ShapePuckAxes,
 ...
 * the same reference when nothing changes — the identity short-circuit
 * lets callers skip downstream work on a no-op.
 */
 ...
 *
 * Pure: no DOM, no I/O. Tested in isolation.
 */
 ...
 *
 * Pure: no DOM, no I/O. Pinned in tests so the renderer-side
 * trigger for the outer ring stays an enforced contract.
 */
...
/**
 * Whether an outcome is a real navigation movement (the puck moved the
 * selection through the ring tree), as opposed to an idle/deadzone frame
 * ('none') or a terminal commit ('activate'/'commitCenter', suppressed in the
 * editor). The editor's live preview uses this to hand the selection from a
 * manual click back to the puck on real movement (last input wins, #447).
 */
export function isMovementOutcome(kind: PuckOutcome['kind']): boolean {
  return kind === 'hover' || kind === 'drill' || kind === 'back' || kind === 'exitToCenter';
}

/**
 * Defensive wrap around a shape plugin's `hitTest` (#107 / #325). The wedge
 * default's sector resolution is pure host code (`axesToSector`) and can't
 * throw; a shape plugin's `hitTest` is third-party code called at frame rate,
 * so it gets a try / catch plus return-shape normalisation. A throw is reported
 * through `onError` (the host throttles + logs; core stays log-free) and folds
 * to null; any non-integer / out-of-range / non-number return also folds to
 * null (no sector hovered), matching the wedge default's `aimed === null`
 * short-circuit. Bound-checked against the layout's node count — the plugin
 * committed to that count when it produced the layout, so the caller doesn't
 * plumb the sector count separately. Shared by the editor's live preview and
 * the native pie runtime (`driveNativeNav`), so it lives in core. Exported
 * (underscore convention) so tests can pin the contract.
 */
export function _safeShapeHitTest(
  ...
  try {
    ...
    onError?.(`hitTest() threw: ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
 ...
 *
 * Pure: no DOM, no I/O. The caller owns the side effects the
