 ...
 * KWin script over DBus. This is the approach needed on KDE because a
...
export class KWinCursorService {
  ...
  /** `scriptDir`: where the helper KWin script file is materialised (the
   *  core passes an XDG state dir); injected so this module stays
   *  runtime-agnostic. */
  constructor(scriptDir: string) {
    this.cursorIface = new CursorInterface(INTERFACE_NAME);
    this.scriptPath = path.join(scriptDir, SCRIPT_FILENAME);
  }
