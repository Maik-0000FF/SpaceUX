...
import { launchCommand } from './builtins/launch-detached.js';
...
export type LoadedPlugin = {
  ...
  /** Optional bridge provider exported by index.js (#288), if any — the editor
   *  calls it to install/uninstall the plugin's companion that lives outside
   *  SpaceUX's tree (FreeCAD's Mod-dir addon). Undefined for plugins without a
   *  bridge. */
...
async function loadOne(dir: string): Promise<LoadedPlugin | { reason: string }> {
  ...
  const provideBridge = typeof mod.provideBridge === 'function' ? mod.provideBridge : undefined;
...
export function validateManifest(value: unknown): string | null {
  ...
  // Declared permissions (#426 Layer 2): optional, but if present must be an
  // array of known values with no duplicates, so an unknown permission can't
  // slip in unnoticed and the user is shown an accurate, deduplicated request.
  if (m.permissions !== undefined) {
    if (
      !Array.isArray(m.permissions) ||
      !m.permissions.every((p) => PLUGIN_PERMISSIONS.includes(p as PluginPermission))
    ) {
      return `manifest field "permissions" must be an array of: ${PLUGIN_PERMISSIONS.join(', ')}`;
    }
    if (new Set(m.permissions).size !== m.permissions.length) {
      return 'manifest field "permissions" must not contain duplicates';
    }
  }
 ...
 *  third-party plugins. `launch` is the same scope-decoupled launcher the
 *  `exec` built-in uses, so plugins get a safe spawn path instead of importing
 *  `node:child_process` (a raw detached spawn stalls logout, #522). `host`
 *  exposes the typed {@link PluginHostCapabilities} surface so plugins that
 *  integrate with host-side state (FreeCAD bridge today, #267) can go through
 *  it instead of importing main-internal modules. */
export function makeActionContext(
  ...
  return {
    ...
    launch: (command) => launchCommand(command, log),
    host,
  };
}
