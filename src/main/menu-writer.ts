...
export async function writeMenuConfig(
  ...
  try {
    ...
    // no file yet (fresh install / never saved); the null default stands.
  }
  ...
  //    the target via the shared atomic write. rename(2) is atomic within one
  //    filesystem, so a reader (the daemon-side loader, another editor) never
  //    sees a half-written file. mkdir -p covers a fresh ~/.config/spaceux.
  try {
    await atomicWriteFile(targetPath, serializeMenuConfig(validation.config));
  } catch (err) {
