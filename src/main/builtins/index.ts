...
import { openFileAction } from './open-file.js';
...
export const BUILTIN_PLUGIN: LoadedPlugin = {
  manifest: {
    ...
    actions: [
      ...
      {
        name: BUILTIN_ACTION.OPEN_FILE,
        label: 'Open file',
        description: 'Open a file or document with the desktop default application (xdg-open).',
        config: {
          path: {
            kind: 'string',
            label: 'File',
            placeholder: '/home/you/Documents/drawing.FCStd',
          },
        },
      },
      {
        ...
        // An explicit empty schema: Cancel takes no config, so the editor shows
        // neither a field nor the raw-JSON fallback (the fallback is only for an
        // action that declares no schema at all). See ActionConfigSchema.
        config: {},
      },
      {
        name: BUILTIN_ACTION.DESKTOP,
        label: 'Desktop control',
        description: 'Adjust volume or brightness, or toggle mute, for the running compositor.',
        config: {
          op: {
            kind: 'string',
            label: 'Control',
            placeholder: 'volume-up | volume-down | mute | brightness-up | brightness-down',
          },
        },
      },
    ],
  ...
  handlers: {
    ...
    [BUILTIN_ACTION.OPEN_FILE]: openFileAction,
    [BUILTIN_ACTION.CANCEL]: cancelAction,
    [BUILTIN_ACTION.DESKTOP]: desktopAction,
  },
};
