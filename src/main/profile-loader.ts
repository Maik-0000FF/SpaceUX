 ...
 * the priority can be unit-tested in isolation. Only a `loaded`
...
function buildProfileWrapper(
  ...
  const wrapper = {
    menu: JSON.parse(serializeMenuConfig(validation.config)) as unknown,
    appearance,
  };
...
export async function writeDeviceProfile(
  ...
  const file = deviceProfilePath(id, dir);
  try {
    await atomicWriteFile(file, built.body);
  } catch (err) {
...
export function writeDeviceProfileSync(
  ...
  const file = deviceProfilePath(id, dir);
  try {
    atomicWriteFileSync(file, built.body);
  } catch {
    // best-effort on the quit path — a failed flush is dropped
  }
}
