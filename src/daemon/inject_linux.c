...
int inject_pointer_open(void)
{
	...
	if (fd < 0) {
		fprintf(stderr,
			"spaceux-daemon: cannot open /dev/uinput for the pointer (%s). "
			"Desktop-mode scrolling disabled — ensure the uinput module is "
			"loaded and the udev rule grants the user access.\n",
			strerror(errno));
		return -1;
	}

	/* Declare buttons and relative axes so libinput classifies this as a
	 * mouse and honours the wheel. We only emit wheel events today, but a
	 * pure-wheel device is sometimes ignored by the input stack, so the
	 * device advertises the full pointer shape (buttons + X/Y + wheels). */
	int ok = 1;
	ok &= (ioctl(fd, UI_SET_EVBIT, EV_KEY) == 0);
	ok &= (ioctl(fd, UI_SET_KEYBIT, BTN_LEFT) == 0);
	ok &= (ioctl(fd, UI_SET_KEYBIT, BTN_RIGHT) == 0);
	ok &= (ioctl(fd, UI_SET_KEYBIT, BTN_MIDDLE) == 0);
	ok &= (ioctl(fd, UI_SET_EVBIT, EV_REL) == 0);
	ok &= (ioctl(fd, UI_SET_RELBIT, REL_X) == 0);
	ok &= (ioctl(fd, UI_SET_RELBIT, REL_Y) == 0);
	ok &= (ioctl(fd, UI_SET_RELBIT, REL_WHEEL) == 0);
	ok &= (ioctl(fd, UI_SET_RELBIT, REL_HWHEEL) == 0);
	ok &= (ioctl(fd, UI_SET_RELBIT, REL_WHEEL_HI_RES) == 0);
	ok &= (ioctl(fd, UI_SET_RELBIT, REL_HWHEEL_HI_RES) == 0);
	if (!ok) {
		fprintf(stderr, "spaceux-daemon: pointer UI_SET_*BIT failed (%s)\n",
			strerror(errno));
		close(fd);
		return -1;
	}
	...
	memset(&usetup, 0, sizeof(usetup));
	usetup.id.bustype = BUS_VIRTUAL;
	/* Same pid.codes test VID as the keyboard; a distinct product id so
	 * the two SpaceUX virtual devices are tellable apart in listings. */
	usetup.id.vendor = 0x1209;
	usetup.id.product = 0xbee6;
	snprintf(usetup.name, UINPUT_MAX_NAME_SIZE, "SpaceUX Virtual Pointer");

	if (ioctl(fd, UI_DEV_SETUP, &usetup) < 0) {
		fprintf(stderr, "spaceux-daemon: pointer UI_DEV_SETUP failed (%s)\n",
			strerror(errno));
		close(fd);
		return -1;
	}
	if (ioctl(fd, UI_DEV_CREATE) < 0) {
		fprintf(stderr, "spaceux-daemon: pointer UI_DEV_CREATE failed (%s)\n",
			strerror(errno));
		close(fd);
		return -1;
	}
	/* Same device-ready settle as the keyboard (see inject_open). */
	usleep(100000);
	return fd;
}
...
/* Sub-notch remainder for the classic REL_WHEEL / REL_HWHEEL axes. The
 * hi-res axis carries full precision on every call; the classic axis
 * steps only once per 120 units, so we accumulate the remainder here and
 * emit a whole notch when it crosses the threshold. Integer division
 * truncates toward zero, so the sign is preserved for upward and
 * downward scrolls alike. */
static int g_wheel_accum;
static int g_hwheel_accum;
...
void inject_scroll(int fd, int dx, int dy)
{
	...
	int emitted = 0;
	if (dy != 0) {
		emit_event(fd, EV_REL, REL_WHEEL_HI_RES, dy);
		g_wheel_accum += dy;
		int notches = g_wheel_accum / 120;
		if (notches != 0) {
			emit_event(fd, EV_REL, REL_WHEEL, notches);
			g_wheel_accum -= notches * 120;
		}
		emitted = 1;
	}
	if (dx != 0) {
		emit_event(fd, EV_REL, REL_HWHEEL_HI_RES, dx);
		g_hwheel_accum += dx;
		int notches = g_hwheel_accum / 120;
		if (notches != 0) {
			emit_event(fd, EV_REL, REL_HWHEEL, notches);
			g_hwheel_accum -= notches * 120;
		}
		emitted = 1;
	}
	if (emitted)
