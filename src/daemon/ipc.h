 ...
 *
 * The daemon talks to the Electron client (and any other connected
 ...
 * close-on-exec, and start accepting. Returns 0 on success or -1 on
 * any failure (errno set). The path is chosen by platform.h's
 * platform_socket_path(); on a successful return l->path holds the
 * bound name so a log line can quote it. */
