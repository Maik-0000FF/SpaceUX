 ...
 *
 * The daemon multiplexes multiple clients (the SpaceUX core, debug CLI,
...
struct sock_state {
	 ...
	 * wasn't running. */
	int inject_fd;
	/* uinput fd of the daemon's relative pointer device, used for
	 * INJECT_SCROLL (#199). -1 when that device could not be opened; the
	 * command then parses and dispatches but inject_scroll no-ops, the
	 * same fail-soft shape as inject_fd. Separate from inject_fd because
	 * it is a distinct virtual device (a pointer, not the keyboard). */
	int inject_ptr_fd;
...
/* Wire the relative pointer fd into the dispatch state. Called once at
 * startup after inject_pointer_open(); -1 disables INJECT_SCROLL handling
 * (the command still parses, inject_scroll no-ops). */
