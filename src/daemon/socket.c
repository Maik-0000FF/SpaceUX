...
static int generate_token(char *out)
{
	...
	out[(size_t)SPACEUX_TOKEN_BYTES * 2] = '\0';
	return 0;
}
...
int sock_init(struct sock_state *s)
{
	...
	/* Pass the listener result through verbatim: 0 / -1 AND the positive
	 * IPC_LISTENER_IN_USE the single-instance probe returns, which main relies
	 * on to exit cleanly instead of starting a second daemon. Don't normalise
	 * this to a 0/-1 shape or that guard silently breaks. */
...
void sock_set_inject_ptr_fd(struct sock_state *s, int fd)
{
	s->inject_ptr_fd = fd;
}
...
int sock_accept(struct sock_state *s)
{
	...
	int hlen = protocol_format_hello(hello, sizeof(hello), SPACEUX_AXIS_COUNT, &s->device,
					 s->inject_fd >= 0, s->led_fd >= 0, s->inject_ptr_fd >= 0,
					 c->auth_token);
	if (hlen > 0)
...
static int apply_cmd(struct sock_state *s, int slot, struct sock_client *c, enum protocol_cmd cmd,
		     const struct protocol_chord *chord, const struct protocol_scroll *scroll,
		     int led_on)
{
	switch (cmd) {
	...
	case PROTO_CMD_INJECT_SCROLL: {
		/* Gated by the same per-connection capability token as
		 * INJECT_CHORD. No leaky bucket here on purpose: scroll is a
		 * benign continuous stream (no shortcuts, nothing destructive)
		 * whose rate is naturally bounded by the puck's report rate, and
		 * a hostile same-UID process has far easier input-injection
		 * paths (it can open its own uinput). A wrong-token line is
		 * dropped with an audit line, as for chords; we deliberately do
		 * not log per scroll event, or the stream would flood the
		 * journal. inject_scroll no-ops when the pointer fd is -1, so
		 * this dispatches cleanly on hosts without the pointer device. */
		if (strncmp(scroll->auth_token, c->auth_token, SPACEUX_TOKEN_HEX_LEN) != 0) {
			fprintf(stderr, "[scroll] drop (bad-token) slot=%d pid=%d\n", slot,
				c->peer_pid);
			return 0;
		}
		inject_scroll(s->inject_ptr_fd, scroll->dx, scroll->dy);
		return 0;
	}
		...
		for (;;) {
			...
			*nl = '\0';
			...
			enum protocol_cmd parsed =
				protocol_parse_command(c->cmd_buf, &chord, &scroll, &led_on);
			if (apply_cmd(s, slot, c, parsed, &chord, &scroll, led_on) < 0) {
				slot_clear(c);
				return -1;
			}
