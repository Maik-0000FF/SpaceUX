...
/** A flat, sanitised command ready to drop into the palette UI. */
export type PaletteCommand = { command: string; label: string; icon?: string };

/** A palette group (a catalog group / FreeCAD workbench) with its commands
 *  already flattened, filtered, and sanitised. */
 ...
 *
 * Per group it:
 *  - keeps only the scoped workbench when `scopeKey` is set (else all groups),
