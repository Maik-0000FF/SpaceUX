...
describe('planets layout()', () => {
  it('returns exactly sectorCount nodes and labels', () => {
    for (const n of [2, 3, 6, 8, 12]) {
      const raw = planets.layout(n, RING_RADII, 'outer');
  ...
  it('handles a zero-sector pie (validator requires nodes.length === sectorCount)', () => {
    ...
    for (const ring of ['inner', 'outer'] as const) {
      const raw = planets.layout(0, RING_RADII, ring);
  ...
  it('handles a single-sector pie (chord-degenerate, ring-thickness cap takes over)', () => {
    const raw = planets.layout(1, RING_RADII, 'outer');
  ...
  it('places sector 0 at "12 o\'clock" (matches the wedge convention so push-forward keeps hovering it)', () => {
    const { nodes } = planets.layout(4, RING_RADII, 'outer');
    expect(nodes[0]!.cx).toBeCloseTo(0, 5);
    expect(nodes[0]!.cy).toBeCloseTo(-RING_RADII.outerLabelRadius, 5);
  });

  it('uses the inner-label radius as the orbit when ring === "inner"', () => {
    const { nodes } = planets.layout(4, RING_RADII, 'inner');
    expect(nodes[0]!.cx).toBeCloseTo(0, 5);
    expect(nodes[0]!.cy).toBeCloseTo(-RING_RADII.innerLabelRadius, 5);
    // All inner-ring planets share the inner orbit.
    for (const n of nodes) {
      expect(Math.hypot(n.cx, n.cy)).toBeCloseTo(RING_RADII.innerLabelRadius, 5);
    }
    // Inner-ring planet radius respects the inner band's thickness cap.
    const innerHalfThickness = (RING_RADII.innerOuterRadius - RING_RADII.innerInnerRadius) / 2;
    for (const n of nodes) {
      expect(n.r).toBeLessThanOrEqual(innerHalfThickness);
    }
  });
  ...
  it('all planets share one orbit radius (no eccentricity, no per-sector drift)', () => {
    const { nodes } = planets.layout(8, RING_RADII, 'outer');
    for (const n of nodes) {
  ...
  it('keeps neighbouring planets from overlapping at high sector counts', () => {
    const { nodes } = planets.layout(12, RING_RADII, 'outer');
  ...
  it('caps planet radius by half the outer ring thickness so a small sector count does not bleed past the band', () => {
    const { nodes } = planets.layout(2, RING_RADII, 'outer');
    const halfThickness = (RING_RADII.outerOuterRadius - RING_RADII.outerInnerRadius) / 2;
    for (const n of nodes) {
  ...
  it('labels sit at the planet centre with middle anchor (host pairs this with dominant-baseline=middle)', () => {
    const { nodes, labels } = planets.layout(6, RING_RADII, 'outer');
    for (let i = 0; i < 6; i++) {
...
describe('planets hitTest()', () => {
  const layout4 = planets.layout(4, RING_RADII, 'outer');
