...
describe('encodeCommand', () => {
  ...
  it('emits INJECT_SCROLL with token and integer dx/dy (truncating fractions)', () => {
    const tok = 'cccccccccccccccccccccccccccccccc';
    expect(encodeCommand({ kind: 'inject-scroll', dx: 0, dy: -3, token: tok })).toBe(
      `INJECT_SCROLL ${tok} 0 -3\n`,
    );
    // The daemon wants integers; a fractional accumulator value is truncated.
    expect(encodeCommand({ kind: 'inject-scroll', dx: 2.9, dy: -1.9, token: tok })).toBe(
      `INJECT_SCROLL ${tok} 2 -1\n`,
    );
  });
