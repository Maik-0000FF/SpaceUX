...
describe('validateManifest permissions', () => {
  it('accepts a manifest that declares none', () => {
    expect(validateManifest(manifestBase())).toBeNull();
  });

  it('accepts known permissions', () => {
    expect(validateManifest(manifestBase({ permissions: ['exec', 'network'] }))).toBeNull();
  });

  it('rejects an unknown permission', () => {
    expect(validateManifest(manifestBase({ permissions: ['exec', 'bogus'] }))).toMatch(
      /permissions/,
    );
  });

  it('rejects a non-array permissions field', () => {
    expect(validateManifest(manifestBase({ permissions: 'exec' }))).toMatch(/permissions/);
  });

  it('rejects duplicate permissions', () => {
    expect(validateManifest(manifestBase({ permissions: ['exec', 'exec'] }))).toMatch(/duplicate/);
  });
});
