...
describe('validateMenuConfig — nested submenus', () => {
  ...
  it('keeps keepOpen=true on the root when the centre has a commit action', () => {
    // The root has both branches (its ring) and an action (the centre commit),
    // so keepOpen is meaningful there: fire the centre action, stay open.
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: {
        label: '',
    ...
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.config.root.keepOpen).toBe(true);
  });

  it('drops keepOpen on a plain/cancel centre that has no action', () => {
    // A centre with no commit action dismisses; keeping the menu open would
    // strand the user, so the flag must not persist.
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: { label: '', keepOpen: true, branches: [{ label: 'x' }] },
    });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.config.root.keepOpen).toBeUndefined();
  });
  ...
  it('keeps iconAuto=true alongside an icon, drops it otherwise', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: {
        label: '',
        branches: [
          { label: 'A', icon: 'data:img', iconAuto: true }, // kept
          { label: 'B', iconAuto: true }, // no icon → dropped (no-op flag)
          { label: 'C', icon: 'data:img', iconAuto: false }, // false → dropped
        ],
      },
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.config.root.branches![0]?.iconAuto).toBe(true);
      expect(r.config.root.branches![1]?.iconAuto).toBeUndefined();
      expect(r.config.root.branches![2]?.iconAuto).toBeUndefined();
    }
  });

  it('rejects a non-boolean iconAuto', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: { label: '', branches: [{ label: 'A', icon: 'data:img', iconAuto: 'yes' }] },
    });
    expect(r.ok).toBe(false);
  });

  it('keeps labelAuto=true alongside a non-empty label, drops it otherwise', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: {
        label: '', // the root may be label-less → labelAuto is a no-op, dropped
        labelAuto: true,
        branches: [
          { label: 'A', labelAuto: true }, // kept (non-empty label)
          { label: 'C', labelAuto: false }, // false → dropped
        ],
      },
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.config.root.labelAuto).toBeUndefined();
      expect(r.config.root.branches![0]?.labelAuto).toBe(true);
      expect(r.config.root.branches![1]?.labelAuto).toBeUndefined();
    }
  });

  it('rejects a non-boolean labelAuto', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: { label: '', branches: [{ label: 'A', labelAuto: 'yes' }] },
    });
    expect(r.ok).toBe(false);
  });

  it('keeps the tri-state iconHidden (true or false) alongside an icon, drops it otherwise (#520)', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: {
        label: '',
        branches: [
          { label: 'A', icon: 'data:img', iconHidden: true }, // kept (force hidden)
          { label: 'B', iconHidden: true }, // no icon → dropped (nothing to control)
          { label: 'C', icon: 'data:img', iconHidden: false }, // kept (force shown)
        ],
      },
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.config.root.branches![0]?.iconHidden).toBe(true);
      expect(r.config.root.branches![1]?.iconHidden).toBeUndefined();
      expect(r.config.root.branches![2]?.iconHidden).toBe(false);
    }
  });

  it('keeps the tri-state labelHidden on a non-empty label or the root, drops it otherwise (#520)', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: {
        label: '', // the root's empty label still renders ✕, which the flag controls → kept
        labelHidden: true,
        branches: [
          { label: 'A', labelHidden: true }, // kept (force hidden)
          { label: 'C', labelHidden: false }, // kept (force shown, an override)
        ],
      },
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(r.config.root.labelHidden).toBe(true);
      expect(r.config.root.branches![0]?.labelHidden).toBe(true);
      expect(r.config.root.branches![1]?.labelHidden).toBe(false);
    }
  });

  it('rejects a non-boolean iconHidden / labelHidden (#515)', () => {
    expect(
      validateMenuConfig({
        version: MENU_CONFIG_VERSION,
        root: { label: '', branches: [{ label: 'A', icon: 'data:img', iconHidden: 'yes' }] },
      }).ok,
    ).toBe(false);
    expect(
      validateMenuConfig({
        version: MENU_CONFIG_VERSION,
        root: { label: '', branches: [{ label: 'A', labelHidden: 'yes' }] },
      }).ok,
    ).toBe(false);
  });

  // Hiding a part keeps its stored value: a node with both hidden still has a
  // non-empty label, so it stays valid and renders as an empty wedge (#515).
  it('accepts a node with both icon and label hidden (empty wedge) (#515)', () => {
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      root: {
        label: '',
        branches: [{ label: 'Mute', labelHidden: true, icon: 'data:img', iconHidden: true }],
      },
    });
    expect(r.ok).toBe(true);
    if (r.ok) {
      const leaf = r.config.root.branches![0]!;
      expect(leaf.label).toBe('Mute');
      expect(leaf.labelHidden).toBe(true);
      expect(leaf.iconHidden).toBe(true);
    }
  });
...
describe('validateMenuConfig — navigation (issue #105)', () => {
  ...
  it('does NOT warn once the higher-priority gesture is set to fire first (lower threshold)', () => {
    // commitCenter outranks drillIn in the runtime resolve order, so setting it
    // strictly below drillIn on the shared axis makes it win cleanly (no fight).
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const r = validateMenuConfig({
      version: MENU_CONFIG_VERSION,
      navigation: {
        drillIn: { inputs: [{ kind: 'axis', axis: 'rz', direction: 'positive', threshold: 200 }] },
        commitCenter: {
          inputs: [{ kind: 'axis', axis: 'rz', direction: 'positive', threshold: 50 }],
        },
      },
      root: { label: '', branches: [{ label: 'x' }] },
    });
...
describe('validateMenuConfig: commitCenter + back at a cancel centre (#404)', () => {
  // Wires the loader's warning to the menu's centre: the same commitCenter+back
  // staggering is a real clash at a non-cancel centre but runtime-safe at a
  // cancel one, so the warning must follow isCancelNode(root).
  const tzNeg = (threshold: number) => ({
    inputs: [{ kind: 'axis', axis: 'tz', direction: 'negative', threshold }],
  });
  const cfg = (cancelCentre: boolean, commit: number, back: number) => ({
    version: MENU_CONFIG_VERSION,
    navigation: { commitCenter: tzNeg(commit), back: tzNeg(back) },
    root: cancelCentre
      ? { label: '', action: { id: builtinAction('cancel') }, branches: [{ label: 'x' }] }
      : { label: '', branches: [{ label: 'x' }] },
  });

  it('does NOT warn for commit-above-back when the centre is a cancel node', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(validateMenuConfig(cfg(true, 200, 150)).ok).toBe(true);
    expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining('both bind axis:tz:negative'));
    warnSpy.mockRestore();
  });

  it('still warns for the same staggering when the centre is not a cancel node', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(validateMenuConfig(cfg(false, 200, 150)).ok).toBe(true);
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('both bind axis:tz:negative'));
    warnSpy.mockRestore();
  });
});
...
describe('DEFAULT_MENU_CONFIG', () => {
  ...
  it('rounds the pie out to four top-level sectors', () => {
    // A blank fourth submenu is shipped purely so the first-run pie looks
    // balanced (four sectors); the count is load-bearing for that intent.
    expect(DEFAULT_MENU_CONFIG.root.branches).toHaveLength(4);
  });

  it('binds the centre to the built-in cancel action', () => {
    expect(isCancelNode(DEFAULT_MENU_CONFIG.root)).toBe(true);
  });

  it('ships a navigation with no input conflicts', () => {
    // The default binds one button to both activate and commitCenter; that pair
    // is disjoint by hover state, so the shipped showcase must not warn about
    // itself (the conflict detector suppresses the safe pair).
    expect(navigationConflicts(DEFAULT_MENU_CONFIG.navigation!)).toEqual([]);
  });
});
...
describe('builtinAction key composition', () => {
  ...
  it('every default-config binding references a known built-in action', () => {
    ...
    const known = new Set<string>([
      ...
      builtinAction(BUILTIN_ACTION.DESKTOP),
    ]);
    // Walk the whole tree: leaves carry a binding (must reference a known
    // built-in), submenus carry `branches` and no action (the Sound submenu),
    // so only assert on nodes that actually have an action.
    ...
    walk(DEFAULT_MENU_CONFIG.root.branches ?? []);
  });
});
