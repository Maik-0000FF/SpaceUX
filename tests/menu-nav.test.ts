...
describe('drillReducer', () => {
  ...
  it('reset is a no-op when already at initial state (identity short-circuit)', () => {
    // The reducer returns the same reference so callers can skip the
    // downstream work. Without this, every MENU_OPEN at idle would
    // still cause a rebuild cascade.
...
describe('isMovementOutcome', () => {
  it('counts ring navigation outcomes as movement', () => {
    for (const kind of ['hover', 'drill', 'back', 'exitToCenter'] as const) {
      expect(isMovementOutcome(kind)).toBe(true);
    }
  });

  it('does not count idle or terminal-commit outcomes as movement', () => {
    for (const kind of ['none', 'activate', 'commitCenter'] as const) {
      expect(isMovementOutcome(kind)).toBe(false);
    }
  });
});
