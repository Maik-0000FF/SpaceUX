...
describe('sanitizePieAppearancePatch', () => {
  ...
  it('keeps the global visibility flags as booleans, drops non-booleans (#518)', () => {
    expect(sanitizePieAppearancePatch({ hideLabels: true, hideIcons: false })).toEqual({
      hideLabels: true,
      hideIcons: false,
    });
    expect(sanitizePieAppearancePatch({ hideLabels: 'yes' })).toEqual({});
    expect(sanitizePieAppearancePatch({ hideIcons: 1 })).toEqual({});
  });
  ...
  it('returns an empty patch when no known keys are present', () => {
    expect(sanitizePieAppearancePatch({ unknownKey: 5, foo: 'bar' })).toEqual({});
  });

  it('passes the blur toggle through, drops a non-boolean', () => {
    expect(sanitizePieAppearancePatch({ blur: true })).toEqual({ blur: true });
    expect(sanitizePieAppearancePatch({ blur: false })).toEqual({ blur: false });
    // Non-boolean (incl. truthy/falsy values) is dropped, not coerced.
    expect(sanitizePieAppearancePatch({ blur: 1 })).toEqual({});
    expect(sanitizePieAppearancePatch({ blur: 'yes' })).toEqual({});
  });
  ...
  it('keeps a valid wedgeStyle, drops an unknown one (#47)', () => {
    expect(sanitizePieAppearancePatch({ wedgeStyle: 'modern' })).toEqual({ wedgeStyle: 'modern' });
    expect(sanitizePieAppearancePatch({ wedgeStyle: 'classic' })).toEqual({
      wedgeStyle: 'classic',
    });
    expect(sanitizePieAppearancePatch({ wedgeStyle: 'neon' })).toEqual({});
    expect(sanitizePieAppearancePatch({ wedgeStyle: 5 })).toEqual({});
  });

  it('keeps a valid wedgeGapStyle, drops an unknown one (#47)', () => {
    expect(sanitizePieAppearancePatch({ wedgeGapStyle: 'wedge' })).toEqual({
      wedgeGapStyle: 'wedge',
    });
    expect(sanitizePieAppearancePatch({ wedgeGapStyle: 'parallel' })).toEqual({
      wedgeGapStyle: 'parallel',
    });
    expect(sanitizePieAppearancePatch({ wedgeGapStyle: 'spiral' })).toEqual({});
  });

  it('keeps + clamps wedgeGap, drops a non-finite one (#47)', () => {
    expect(sanitizePieAppearancePatch({ wedgeGap: 0.03 })).toEqual({ wedgeGap: 0.03 });
    expect(sanitizePieAppearancePatch({ wedgeGap: 9 })).toEqual({ wedgeGap: PIE_WEDGE_GAP_MAX });
    expect(sanitizePieAppearancePatch({ wedgeGap: -1 })).toEqual({ wedgeGap: PIE_WEDGE_GAP_MIN });
    expect(sanitizePieAppearancePatch({ wedgeGap: NaN })).toEqual({});
  });

  it('keeps + clamps wedgeHoverOffset, drops a non-finite one (#47)', () => {
    expect(sanitizePieAppearancePatch({ wedgeHoverOffset: 0.04 })).toEqual({
      wedgeHoverOffset: 0.04,
    });
    expect(sanitizePieAppearancePatch({ wedgeHoverOffset: 9 })).toEqual({
      wedgeHoverOffset: PIE_WEDGE_HOVER_MAX,
    });
    expect(sanitizePieAppearancePatch({ wedgeHoverOffset: -1 })).toEqual({
      wedgeHoverOffset: PIE_WEDGE_HOVER_MIN,
    });
    expect(sanitizePieAppearancePatch({ wedgeHoverOffset: NaN })).toEqual({});
  });
