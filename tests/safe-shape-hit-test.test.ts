...
import { _safeShapeHitTest } from '../src/core/menu-nav';
import type {
  ShapeLayout,
  ShapePluginModule,
  ShapePuckAxes,
...
describe('_safeShapeHitTest', () => {
  ...
  it('catches a throw, folds to null, and reports the reason via onError', () => {
    const mod = makeModule(() => {
      throw new Error('plugin bug');
    });
    const reasons: string[] = [];
    expect(
      _safeShapeHitTest(mod, FAKE_RING_RADII, FAKE_LAYOUT, ZERO_AXES, (r) => reasons.push(r)),
    ).toBeNull();
    expect(reasons).toHaveLength(1);
    expect(reasons[0]).toContain('hitTest() threw');
    expect(reasons[0]).toContain('plugin bug');
  });

  it('does not call onError when the plugin returns normally', () => {
    const reasons: string[] = [];
    _safeShapeHitTest(
      makeModule(() => 99),
      FAKE_RING_RADII,
