...
describe('serializeMenuConfig', () => {
  ...
  it('serializes and round-trips the visibility flags (#515)', () => {
    ...
    const json = serializeMenuConfig(cfg);
    expect(json).toContain('"labelHidden": true');
    expect(json).toContain('"iconHidden": true');
    const r = validateMenuConfig(JSON.parse(json));
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.config).toEqual(cfg);
  });

  it('serializes and round-trips a force-shown override (#520)', () => {
    const cfg: MenuConfig = {
      version: 1,
      root: {
        label: '',
        branches: [{ label: 'Mute', labelHidden: false, icon: 'data:img', iconHidden: false }],
      },
    };
    const json = serializeMenuConfig(cfg);
    expect(json).toContain('"labelHidden": false');
    expect(json).toContain('"iconHidden": false');
    const r = validateMenuConfig(JSON.parse(json));
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.config).toEqual(cfg);
  });
